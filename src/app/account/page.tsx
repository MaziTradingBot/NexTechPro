...
export default function AccountPage() {
    ...
    return (
      <div className="wrap py-20">
        <div className="mx-auto max-w-md rounded-3xl border border-[var(--border)] bg-white p-10 text-center">
          ...
          <h1 className="mt-5 text-2xl font-bold text-slate-900">{t("common.account")}</h1>
          ...
          <div className="mt-6 flex justify-center gap-3">
            ...
            <Link href="/register" className="rounded-xl border border-[var(--border)] bg-white px-6 py-3 text-sm font-semibold text-slate-700 hover:bg-slate-50">
              {t("auth.signUp")}
            </Link>
          </div>
        </div>
  ...
  return (
    <div className="wrap py-8">
      <h1 className="text-3xl font-extrabold tracking-tight text-slate-900">{t("account.title")}</h1>
      ...
      <div className="mt-8 grid gap-6 lg:grid-cols-[320px_1fr]">
        ...
        <div className="h-fit rounded-2xl border border-[var(--border)] bg-white p-6">
          ...
          <button
            ...
            className="mt-6 flex w-full items-center justify-center gap-2 rounded-xl border border-[var(--border)] py-3 text-sm font-semibold text-slate-700 transition hover:bg-rose-50 hover:text-rose-600"
          >
        ...
        <div>
          ...
          {myOrders.length === 0 ? (
            ...
            <div className="space-y-3">
              {myOrders.map((o) => (
                <div key={o.id} className="rounded-2xl border border-[var(--border)] bg-white p-5">
                  <div className="flex flex-wrap items-center justify-between gap-2">
                    <div>
                      <span className="font-mono font-bold text-brand-700">{o.id}</span>
                  ...
                  <div className="mt-3 flex items-center justify-between border-t border-[var(--border)] pt-3 text-sm">
                    ...
                    <span className="font-bold text-slate-900">{formatPrice(o.total, locale)}</span>
                  </div>
                </div>
              ))}
            </div>
